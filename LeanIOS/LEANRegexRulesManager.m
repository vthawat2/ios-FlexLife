...
- (NSDictionary *)matchesWithUrlString:(NSString *)urlString {
    if ([GoNativeAppConfig sharedAppConfig].calendarEnabled) {
        NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", @"data:text/calendar;.*"];
        if ([predicate evaluateWithObject:urlString]) {
            return @{ @"matches": @YES, @"mode": @"internal" };
        }
    }
